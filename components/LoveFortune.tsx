...
const LoveFortune: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const request = fetch('/api/love-fortune', {
      ...
      if (error.name === 'AbortError') {
        ...
        console.error('获取爱情运势预测失败:', error);
        setFortune(error.message || '抱歉，获取爱情运势预测结果时出现错误，请稍后重试。');
      }
    } finally {
      if (isActive) {
        setLoading(false);
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <Typography variant="h3" component="h3" className="text-center mb-2">爱情运势预测</Typography>
      <Typography variant="body1" className="text-center text-gray-600 mb-6">探索你的爱情命运，获得感情指引</Typography>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            ...
            <FormControl fullWidth required>
              <InputLabel id="relationship-status-label">目前的情感状态</InputLabel>
              <Select
              ...
              >
                ...
                <MenuItem value="single">单身</MenuItem>
                <MenuItem value="in_relationship">恋爱中</MenuItem>
                <MenuItem value="married">已婚</MenuItem>
              </Select>
            </FormControl>
      ...
      {state.loading && <MysticLoading text="爱情运势预测结果..." />}
