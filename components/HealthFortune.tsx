...
const HealthFortune: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const request = fetch('/api/health-fortune', {
      ...
      if (error.name === 'AbortError') {
        ...
        console.error('获取健康运势预测失败:', error);
        setFortune(error.message || '抱歉，获取健康运势预测结果时出现错误，请稍后重试。');
      }
    } finally {
      if (isActive) {
        setLoading(false);
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <Typography variant="h3" component="h3" className="text-center mb-2">健康运势预测</Typography>
      <Typography variant="body1" className="text-center text-gray-600 mb-6">探索你的健康状况，获得养生指引</Typography>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            ...
            <FormControl fullWidth required>
              <InputLabel id="health-status-label">最近的健康状况</InputLabel>
              <Select
                labelId="health-status-label"
                fullWidth
                value={state.formData.healthStatus || ""}
                onChange={(e) => setFormData({ ...state.formData, healthStatus: e.target.value })}
                sx={{ borderRadius: '4px' }}
                label="最近的健康状况"
                tabIndex={0}
                MenuProps={{
              ...
              >
                ...
                <MenuItem value="tired">疲劳</MenuItem>
                <MenuItem value="stressed">压力大</MenuItem>
                <MenuItem value="good">良好</MenuItem>
                <MenuItem value="excellent">非常好</MenuItem>
              </Select>
            </FormControl>
      ...
      {state.loading && <MysticLoading text="健康运势预测结果..." />}
